...
class RGBlock(nn.Module):
    "Remove the residual connection between x and v, transforming it into a unidirectional gated structure."
...
class ChannelAttentionModule(nn.Module):
    ...
    def __init__(self, in_channels):
        ...
        self.sigmoid = nn.Sigmoid()
        self.conv_expand = nn.Conv2d(in_channels // 2, in_channels, kernel_size=1, stride=1, padding=0)

    def forward(self, x):
        global_average = self.global_pool(x)
        global_average = self.conv(global_average)
        global_average = self.sigmoid(global_average)
        global_average = self.conv_expand(global_average)  # 恢复通道维度
        return x * global_average

class HybridFeatureIntegrationBlock(nn.Module):
    "HFIB Block"
    def __init__(self, in_channels):
        super(HybridFeatureIntegrationBlock, self).__init__()
        self.depthwise_conv = nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=1, padding=1, groups=in_channels)
        self.pointwise_conv = nn.Conv2d(in_channels, in_channels, kernel_size=1, stride=1, padding=0)
